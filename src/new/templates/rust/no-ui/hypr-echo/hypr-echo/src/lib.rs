...
impl HyprEchoState {
    ...
    async fn echo(&self, req: HyprEchoReq) -> HyprEchoResp {
        if req.payload == "echo" {
            HyprEchoResp { payload: "echo".to_string() }
        } else {
    ...
    #[ws]
    fn ws_echo(&mut self, channel_id: u32, _message_type: WsMessageType, _blob: LazyLoadBlob) {
        // Respond with "echo"
        send_ws_push(
            channel_id,
            WsMessageType::Text,
            LazyLoadBlob {
                ...
                bytes: serde_json::to_vec("Echo from server").unwrap(),
            },
        );
    }
}

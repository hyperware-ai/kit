    ...
    for item in &impl_item.items {
        if let ImplItem::Fn(method) = item {
            ...
            if has_remote || has_local || has_http || has_init || has_ws || has_ws_client || has_timer {
                ...
                return Err(eyre!(
                         "Method '{}' in the #[hyperprocess] impl block is missing a required attribute ([remote], [local], [http], [init], [ws], [ws_client] or [timer]). Only methods with these attributes should be included.",
                         method_name
                     ));
            }
        }
